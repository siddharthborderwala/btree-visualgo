            ...
            if (!root.isLastInternalNode()) {
                for (int i = 0; i < root.getChildren().size(); i++) {
                    ...
                    if ((double) i > ((double) root.getSize()) / 2) {
                        // TODO: fix
                        startX2 = startX
                                + ((bTree.getOrder() - 1) * (bTree.getHeight(root.getChild(i)) - 1) * rectangleWidth >> 1);
                        endX = startX2 + ((double) root.getChild(i).getSize()) / 2 * rectangleWidth;
                    } else if ((double) i < ((double) root.getSize()) / 2) {
                        endX = startX - ((bTree.getOrder() - 1) * (bTree.getHeight(root.getChild(i)) - 1) * rectangleWidth >> 1)
                ...
                while (i < currentNode.getSize()) {
                    ...
                    if (currentNode.getKey(i).equals(key)) {
                        ...
                        if ((double) i < ((double) currentNode.getSize()) / 2) {
                            x -= ((bTree.getOrder() - 1) * (bTree.getHeight(currentNode.getChild(i)) - 1) * rectangleWidth >> 1) - ((double) currentNode.getChild(i).getSize()) * rectangleWidth;
                        } else {
